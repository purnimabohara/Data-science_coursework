...
setwd("/Users/ACER/Purnima_DataScience")
...
HousePricesclean %>%
  ...
  group_by(County) %>%
  summarise(AveragePrice = mean(Price)) %>%
  ggplot(aes(x = factor(County), y = AveragePrice)) +
  geom_line(linewidth = 1.5, color = "lightgrey") +
  geom_text(aes(label = euro(AveragePrice)), vjust = -0.85) +
  scale_y_continuous(breaks = seq(0, 700000, 50000), label = euro) +
  scale_x_discrete(labels = HousePricesclean$County) +
  geom_point(size = 2, color = "steelblue") +
  labs(title = "2020-2022 Average house prices")
